...
- **Auth:** Better Auth (JWT + Bearer plugins) via raw fetch from mobile, Bearer token in Preferences, social login via @capgo/capacitor-social-login (Google + Apple idToken handoff)
...
- [ ] M006: No-Reload Language Switching — Wire paraglide's overwriteGetLocale/overwriteSetLocale with reactive Svelte 5 state and {#key locale} so language changes apply instantly without page reload.
